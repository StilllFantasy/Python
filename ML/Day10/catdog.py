...
# 读取数据
root = 'Cats_Dogs'
dataset_train = datasets.ImageFolder('D:\\DATA\\Cat_Dog\\lab_data', transform)
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device).float().reshape(10, 1)
        optimizer.zero_grad()
        output = model(data)
        # print(output)
